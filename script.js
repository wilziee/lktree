...
function initIntroSplash() {
    ...
    // Trigger klik user untuk membuka kunci audio browser secara paksa
    startOverlay.addEventListener('click', () => {
        // Hilangkan overlay dengan efek fade-out transparan
        startOverlay.style.opacity = '0';
        setTimeout(() => { startOverlay.style.display = 'none'; }, 500);

        // Putar audio dan video secara bersamaan (Dijamin bersuara)
        introVideo.play().catch(err => console.log(err));
        introAudio.play().catch(err => console.log(err));

        // Jalankan timer kemunculan tombol Skip setelah media mulai
        setTimeout(() => {
            if (!isTransitioning) {
